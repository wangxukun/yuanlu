      ...
      >
        ...
        <div className="w-full flex items-center justify-between gap-2 md:gap-6">
          ...
          <div className="flex-1 flex flex-col items-center gap-1 min-w-0">
            <div className="flex items-center gap-1.5 sm:gap-4 md:gap-5">
              ...
              <button
                ...
                className="text-ink-400 hover:text-primary-600 font-bold text-[10px] sm:text-xs transition-colors w-6 sm:w-8 text-center md:hidden"
                title="播放速度"
              >
                {playbackRate}x
              </button>
              ...
              <button
                onClick={togglePlay}
                className="w-9 h-9 md:w-10 md:h-10 bg-primary-600 text-white rounded-full flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 transition-transform"
              >
                {isPlaying ? (
                  <span
              ...
              <button
                ...
                className={`text-ink-400 hover:text-primary-600 transition-colors md:hidden ${isLoadingLyrics ? "animate-pulse" : ""}`}
                title="全屏沉浸模式"
              >
            ...
            <div className="hidden md:flex items-center gap-3 w-full max-w-xl">
              ...
              <div className="flex-1 h-1 bg-base-200 rounded-full relative group/progress">
                <div
                  ...
                  style={{ width: `${progressPercent}%` }}
                >
                  <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-white border-2 border-primary-600 rounded-full shadow-md opacity-100 transition-transform scale-75 group-hover/progress:scale-100 active:scale-110" />
                </div>
                <input
                  type="range"
                  min="0"
              ...
              <span className="text-[10px] text-ink-400 font-medium w-8">
                {formatTime(duration)}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 md:gap-4 w-auto md:w-1/4 md:min-w-[80px]">
            ...
            <button
              onClick={cyclePlaybackRate}
              className="text-ink-400 hover:text-primary-600 font-bold text-xs transition-colors w-8 text-center hidden md:block"
              title="播放速度"
            >
              {playbackRate}x
            </button>
            ...
            <button
              ...
              className={`transition-colors shrink-0 ${isPlaylistOpen ? "text-primary-600 dark:text-primary-400" : "text-ink-400 hover:text-primary-600"}`}
              title="播放列表"
            >
            ...
            <button
              onClick={closePlayer}
              className="text-ink-400 hover:text-error-500 transition-colors shrink-0"
              title="关闭播放器"
            >
              ...
              {playlist.map((ep, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-xs font-semibold truncate ${currentEpisode.episodeid === ep.episodeid ? "text-primary-600 dark:text-primary-400" : "text-ink-700 dark:text-ink-300"}`}
                    >
                      {ep.title}
                    </p>
