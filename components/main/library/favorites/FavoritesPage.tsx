...
const FavoritesPage: React.FC<FavoritesPageProps> = ({
  ...
  console.log("isPending is placeholder", isPending);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-500 font-sans">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 border-b border-gray-100 dark:border-slate-800 pb-8 transition-colors">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-slate-100 flex items-center">
        ...
        <div className="flex bg-gray-100 dark:bg-slate-800 p-1 rounded-2xl w-full md:w-auto transition-colors">
          <button
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm text-sm text-gray-900 dark:text-slate-100 dark:placeholder-slate-500"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
        </div>
        <button className="flex items-center space-x-2 px-6 py-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl text-gray-600 dark:text-slate-300 font-bold hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors shadow-sm text-sm">
          <Filter size={18} />
          <span>排序</span>
        </button>
      </div>
      ...
      {activeTab === "podcasts" ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredPodcasts.map((series) => (
              <div
                ...
                className="group bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer"
              >
                ...
                <div className="p-5 space-y-3">
                  ...
                  <h3
                    className="font-bold text-gray-900 dark:text-slate-100 line-clamp-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors"
                    title={series.title}
                  >
                    {series.title}
                  </h3>
                  <p className="text-xs text-gray-500 dark:text-slate-400 mt-1 flex items-center">
                    {series.author && (
                      <span className="badge badge-ghost dark:bg-slate-800 dark:text-slate-300 badge-xs mr-2 border-none">
                        {series.author}
                      </span>
                    )}
                  </p>
                  <div className="flex items-center justify-between pt-3 border-t border-gray-50 dark:border-slate-800 text-[11px] text-gray-400 dark:text-slate-500 font-medium">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden divide-y divide-gray-50 dark:divide-slate-800 transition-colors">
          {filteredEpisodes.map((episode) => (
            <div
              key={episode.id}
              onClick={() => handlePlayEpisode(episode.id)}
              className="group flex items-center p-4 sm:p-6 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20 transition-all cursor-pointer relative"
            >
              <div className="w-32 h-18 sm:w-32 sm:h-18 rounded-2xl overflow-hidden flex-shrink-0 shadow-sm bg-gray-100 dark:bg-slate-800">
                <img
              ...
              <div className="ml-4 sm:ml-6 flex-1 min-w-0 pr-8">
                ...
                <h4 className="text-lg font-bold text-gray-900 dark:text-slate-100 truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                  {episode.title}
                </h4>
              ...
              <div className="hidden md:flex items-center space-x-8 mr-12 text-gray-500 dark:text-slate-400">
...
const EmptyState = ({ type }: { type: "podcasts" | "episodes" }) => (
  <div className="py-20 text-center bg-white dark:bg-slate-900 rounded-[32px] border-2 border-dashed border-gray-100 dark:border-slate-800 transition-colors">
    ...
    <a
      href="/discover"
      className="bg-indigo-600 text-white px-8 py-3 rounded-full font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 dark:shadow-none inline-block"
    >
      去发现
    </a>
  </div>
