import Image from "next/image";
import { fetchEpisodes } from "@/lib/data";
import { DeleteEpisodeBtn, UpdateEpisodeBtn } from "@/components/admin/buttons";
import EpisodeStatus from "@/components/admin/episodes/status";
import IsExclusive from "@/components/admin/episodes/IsExclusive";
import { Episode } from "@/core/episode/episode.entity";

export default async function EpisodesTableBackup() {
  const episodes = ((await fetchEpisodes()) as Episode[]) || [];

  return (
    <div className="mt-6 flow-root">
      <div className="inline-block min-w-full align-middle">
        <div className="rounded-lg bg-gray-50 p-2 md:pt-0">
          <div className="md:hidden">
            {episodes?.map((episode) => (
              <div
                key={episode.episodeid}
                className="mb-2 w-full rounded-md bg-white p-4"
              >
                <div className="flex items-center justify-between border-b pb-4">
                  <div>
                    <div className="mb-2 flex items-center">
                      {episode.title}
                    </div>
                    <p className="text-sm text-gray-500">
                      {episode.podcast.title}
                    </p>
                  </div>
                  <div className="h-8 w-28">按钮在此处</div>
                </div>
                <div className="flex w-full items-center justify-between pt-4">
                  <div>{episode.podcast.podcastid}</div>
                  <div className="flex justify-end gap-2">
                    <Image
                      src={episode.coverUrl || "/static/images/1.png"}
                      alt={`${episode.title}封面`}
                      width={100}
                      height={60}
                    />
                  </div>
                </div>
              </div>
            ))}
          </div>
          <table className="hidden min-w-full text-gray-900 md:table">
            <thead className="rounded-lg text-left text-sm font-normal">
              <tr>
                <th scope="col" className="px-7 py-5 ">
                  封面图片
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  播客
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  剧集标题
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  音频
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  发布日期
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  状态
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  权限
                </th>
                <th scope="col" className="relative py-3 pl-6 pr-3">
                  <span className="sr-only">Edit</span>
                </th>
              </tr>
            </thead>
            <tbody className="bg-white">
              {episodes?.map((episode) => (
                <tr
                  key={episode.episodeid}
                  className="w-full border-b py-3 text-sm last-of-type:border-none [&:first-child>td:first-child]:rounded-tl-lg [&:first-child>td:last-child]:rounded-tr-lg [&:last-child>td:first-child]:rounded-bl-lg [&:last-child>td:last-child]:rounded-br-lg"
                >
                  <td className="whitespace-nowrap py-3 pl-6 pr-3">
                    <div className="flex items-center gap-3">
                      {/* 添加relative容器和样式调整 */}
                      <div className="relative h-[60px] w-[100px]">
                        <Image
                          src={episode.coverUrl || "/static/images/1.png"}
                          alt={`${episode.title}封面`}
                          fill // 填充整个父容器
                          sizes="(max-width: 768px) 100vw, 50vw"
                          className="object-cover" // 添加填充方式
                        />
                      </div>
                    </div>
                  </td>
                  <td className="whitespace-nowrap px-3 py-3">
                    {episode.podcast.title}
                  </td>
                  <td className="whitespace-nowrap px-3 py-3">
                    {episode.title}
                  </td>
                  <td className="whitespace-nowrap px-3 py-3">
                    <audio controls>
                      <source src={episode.audioUrl} type="audio/mpeg" />
                      Your browser does not support the audio element.
                    </audio>
                  </td>
                  <td className="whitespace-nowrap px-3 py-3">
                    {new Date(episode.publishAt).toLocaleDateString()}
                  </td>
                  <td className="px-3 py-3">
                    <EpisodeStatus status={episode.status} />
                  </td>
                  <td className="px-3 py-3 align-middle">
                    <IsExclusive isExclusive={episode.isExclusive} />
                  </td>
                  <td className="whitespace-nowrap px-3 py-3">
                    <div className="flex justify-end gap-3">
                      <UpdateEpisodeBtn id={episode.episodeid} />
                      <DeleteEpisodeBtn
                        id={episode.episodeid}
                        coverFileName={episode.coverFileName || ""}
                        audioFileName={episode.audioFileName || ""}
                        subtitleEnFileName={episode.subtitleEnFileName || ""}
                        subtitleZhFileName={episode.subtitleZhFileName || ""}
                      />
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
