...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const fileName = `yuanlu/podcastes/episodes/covers/${timestamp}_${Math.random().toString(36).substring(2)}.${coverFile.name.split(".").pop()}`;
    const { fileUrl: coverUrl } = await uploadToOSS(buffer, fileName);
