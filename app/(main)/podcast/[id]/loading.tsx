...
export default function PodcastDetailLoading() {
  return (
    <div className="min-h-screen bg-ink-50 dark:bg-ink-950 font-sans pb-24 relative w-full overflow-x-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-[380px] overflow-hidden -z-10 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-ink-50/80 dark:via-ink-950/80 to-ink-50 dark:to-ink-950"></div>
      </div>

      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 relative z-10 space-y-8">
        {/* Header Section Skeleton */}
        <div className="bg-white/80 dark:bg-ink-900/80 backdrop-blur-xl rounded-[2rem] p-6 lg:p-8 border border-ink-100 dark:border-ink-800/50 shadow-sm flex flex-col md:flex-row gap-8 items-start md:items-center">
          {/* Large Square Cover Skeleton */}
          <div className="w-40 h-40 md:w-44 md:h-44 rounded-2xl bg-ink-200 dark:bg-ink-800 animate-shimmer flex-shrink-0 mx-auto md:mx-0 shadow-md" />

          {/* Podcast Info Text Skeleton */}
          <div className="flex-1 space-y-4 w-full text-center md:text-left">
            {/* Platform indicator */}
            <div className="h-3 w-16 rounded bg-ink-200 dark:bg-ink-800 animate-shimmer mx-auto md:mx-0" />
            {/* Title */}
            <div className="h-7 w-3/4 rounded bg-ink-200 dark:bg-ink-800 animate-shimmer mx-auto md:mx-0" />

            {/* Tags Pills Row */}
            <div className="flex flex-wrap gap-2 justify-center md:justify-start">
              <div className="h-6 w-16 rounded-full bg-ink-200 dark:bg-ink-800 animate-shimmer" />
              <div className="h-6 w-20 rounded-full bg-ink-200 dark:bg-ink-800 animate-shimmer" />
            </div>

            {/* Counts indicators Row */}
            <div className="flex items-center gap-4 justify-center md:justify-start text-xs pt-1">
              <div className="h-4 w-20 rounded bg-ink-200 dark:bg-ink-800 animate-shimmer" />
              <div className="h-4 w-16 rounded bg-ink-200 dark:bg-ink-800 animate-shimmer" />
            </div>

            {/* Follow/Subscribe Button Skeleton */}
            <div className="pt-2 flex justify-center md:justify-start">
              <div className="h-10 w-28 rounded-full bg-ink-200 dark:bg-ink-800 animate-shimmer" />
            </div>
          </div>
        </div>

        {/* Description Card Skeleton */}
        <div className="bg-white/80 dark:bg-ink-900/80 backdrop-blur-xl rounded-[2rem] p-6 lg:p-8 border border-ink-100 dark:border-ink-800/50 shadow-sm space-y-3">
          <div className="h-5 w-24 rounded bg-ink-200 dark:bg-ink-800 animate-shimmer" />
          <div className="h-4 w-full rounded bg-ink-200 dark:bg-ink-800 animate-shimmer" />
          <div className="h-4 w-5/6 rounded bg-ink-200 dark:bg-ink-800 animate-shimmer" />
          <div className="h-4 w-2/3 rounded bg-ink-200 dark:bg-ink-800 animate-shimmer" />
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between px-2">
            ...
            <div className="h-5 w-16 rounded bg-ink-200 dark:bg-ink-800 animate-shimmer" />
          </div>
          ...
          <div className="space-y-4">
            {Array.from({ length: 4 }).map((_, index) => (
              ...
              >
                ...
                <div className="w-8 h-8 rounded-full bg-ink-200 dark:bg-ink-800 animate-shimmer flex-shrink-0 flex items-center justify-center" />
              </div>
            ))}
          </div>
        </div>
