    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans pb-24 relative w-full overflow-x-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 lg:pt-12 relative z-10 space-y-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          {/* Back button pill */}
          <div className="flex items-center gap-2 text-slate-400 shrink-0 w-20">
            <div className="p-1.5 rounded-full bg-slate-200 dark:bg-slate-800 w-8 h-8 animate-shimmer" />
            <div className="h-4 w-8 rounded bg-slate-200 dark:bg-slate-800 animate-shimmer" />
          </div>

          {/* Podcast Title in Header */}
          <div className="flex-1 space-y-2 min-w-0">
            <div className="h-6 w-1/3 rounded bg-slate-200 dark:bg-slate-800 animate-shimmer" />
            <div className="h-3 w-16 rounded bg-slate-200 dark:bg-slate-800 animate-shimmer" />
          </div>
        </div>

        {/* Toolbar: Search + Sort Skeleton */}
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-3 sm:p-4 shadow-sm border border-slate-200/50 dark:border-slate-800/50 mb-6">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            {/* Search input mock */}
            <div className="relative flex-1">
              <div className="h-10 w-full rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200/50 dark:border-slate-800/50 animate-shimmer" />
            </div>

            {/* Sort select mock */}
            <div className="h-10 w-full sm:w-36 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200/50 dark:border-slate-800/50 animate-shimmer" />
          </div>
        </div>

        {/* Episodes List Container Skeleton */}
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2rem] p-4 sm:p-6 lg:p-8 shadow-sm border border-slate-200/50 dark:border-slate-800/50">
          <div className="space-y-4">
            {Array.from({ length: 4 }).map((_, index) => (
              ...
              >
                ...
                <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-800 animate-shimmer flex-shrink-0" />
              </div>
            ))}
          </div>
        </div>
