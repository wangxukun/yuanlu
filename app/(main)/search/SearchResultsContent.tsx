  ...
  if (!query.trim()) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <span className="material-symbols-outlined text-6xl text-ink-300 dark:text-ink-600 mb-4">
          podcast_search
        </span>
        <p className="text-ink-500 dark:text-ink-400 text-lg font-medium">
          输入关键词开始搜索
        </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {results.map((podcast) => (
          <Link href={`/podcast/${podcast.podcastid}`} key={podcast.podcastid}>
            <div className="rounded-[1rem] transition-all group cursor-pointer hover:shadow-lg hover:shadow-ink-200/50 dark:hover:shadow-black/20 hover:-translate-y-1 duration-300">
              {/* Cover */}
              <div className="aspect-square rounded-[1rem] overflow-hidden mb-4 relative bg-ink-200 dark:bg-ink-800">
                <Image
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center">
                  <div className="w-14 h-14 bg-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0 shadow-xl">
                    <span
                      className="material-symbols-outlined text-primary-600 text-3xl"
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {podcast.tags.slice(0, 2).map((tag) => (
                  <span
                    key={tag.id}
                    className="text-[10px] text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/30 px-2 py-0.5 rounded-full font-medium"
                  >
                    {tag.name}
                  </span>
                ))}
