      ...
      <div className="px-6 lg:px-8 py-10 max-w-7xl mx-auto">
        ...
        <section className="mb-16">
          ...
          <div className="flex lg:grid lg:grid-cols-4 gap-6 overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 -mx-6 px-6 lg:mx-0 lg:px-0 scrollbar-none">
            {trendingPodcasts.slice(0, 4).map((podcast, index) => {
              ...
              const badge = (
                <div
                  className={`${rankColors[index]} w-8 h-8 rounded-full flex items-center justify-center font-black text-sm italic shadow-sm`}
                >
                  0{index + 1}
                </div>
              );
              ...
              const playsField = (
                <span className="flex items-center gap-1" key="plays">
              ...
              return (
                <PodcastCard
                  ...
                  title={podcast.title}
                  platform={podcast.platform || "热门趋势"}
                  badge={badge}
                  metaFields={[playsField]}
        ...
        <HorizontalScrollContainer title="为您推荐">
          {editorPicks.map((podcast) => {
            const categoryTag = (
              <span
                key="cat"
                className="text-xs text-primary-600 dark:text-primary-400 font-medium px-2 py-0.5 bg-ink-100 dark:bg-ink-800 rounded-full truncate max-w-full"
              >
                {podcast.category}
              </span>
            );
            ...
            return (
              <PodcastCard
                ...
                coverUrl={podcast.coverUrl}
                title={podcast.title}
                platform={podcast.platform || "精品推荐"}
                metaFields={[`${podcast.episodeCount} 集`, categoryTag]}
        ...
        <HorizontalScrollContainer title="新节目">
          {newPodcasts.map((podcast) => {
            const badge = (
              <div className="px-3 py-1 bg-primary-600 text-white text-[10px] font-bold rounded-full shadow-lg">
                New
              </div>
            );
            const categoryTag = (
            ...
            return (
              <PodcastCard
                ...
                coverUrl={podcast.coverUrl}
                title={podcast.title}
                platform={podcast.platform || "新作推荐"}
                badge={badge}
                metaFields={[`${podcast.episodeCount} 集`, categoryTag]}
                className="flex-none w-64"
              />
